...
int photo_threshold = 350;                                                //PHOTO THRESHOLD
int photo_deadband = 100;
...
//  Serial.println(is_in_the_dark());
  ...
  if(light_is_on()){
    ...
    while(light_is_on() && millis() <= light_on_time + 600000 / clock_prescaler){
...
//   Serial.println(MODE);
  ...
  //return; //use this to turn on/ off the mode gun response
  ...
  if (num_pulses > num_pulses_max) {
    ...
    long time_in = millis();
    int time_out_delay = 30000 / clock_prescaler;
    ...
    delay(transmit_pulses * 48 / clock_prescaler * 4);
...
void go_into_low_power(int _sleep_time) {
  ...
  //shut off all LowPower calls because of red blinky error state. replacing with regular delay instead)

  delay(_sleep_time / clock_prescaler);
...
//**********************************************************************
boolean darkness() {
  return analogRead(photo_pin) < photo_threshold - photo_deadband;
}

//**********************************************************************

void setup_timer1() {  // Used for transmit charge circuit
